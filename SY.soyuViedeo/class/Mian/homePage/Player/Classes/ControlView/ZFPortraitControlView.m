...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.topToolView addSubview:self.forScreenBtn];////投屏按钮
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(netWorkChang:) name:KVEYSTRING object:nil];
    }
    return self;
}
...
- (void)layoutSubviews {
    ...
    min_x = 35;
    min_y = iPhoneX?30:15;
    min_w = SCREENH_HEIGHT-70;
    min_h = 40;
    self.titleLabel.frame = CGRectMake(min_x, min_y, min_w, min_h);
    
    min_x  = self.topToolView.right - 50;
    min_y = self.titleLabel.zf_y;
    min_h = 40;
    min_w = 40;
...
-(UIButton *)forScreenBtn{
    if (!_forScreenBtn) {
        ...
        _forScreenBtn.clickAction = ^(UIButton *button) {
            if (weakSelf.forScreen) {
                weakSelf.forScreen();
            }
        };
    }
...
-(void)netWorkChang:(NSNotification *)noti{
